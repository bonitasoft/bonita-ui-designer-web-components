...
import { removeComment } from '@bonitasoft/uid-common/dist/src/test/test-common-utils'; // eslint-disable-line

let uidTitle: any;

beforeEach(async () => {
  uidTitle = await fixture(html` <uid-title></uid-title> `);
});
...
describe('UidTitle', () => {
  ...
  it('Should display a h2 title', async () => {
    uidTitle = await fixture(
      html` <uid-title text="A custom title" level="level 2"></uid-title> `
    );
